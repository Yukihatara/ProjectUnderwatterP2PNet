...
def log_event_send(node_id, msg_type, targets, delay, start_time):
    if msg_type == 'Hello':
        return
...
# def process_request_packets(msg, mode): # mode: SndPck or RtrPck
        ...
        if node_id in send_storage: # Проверка на наличии пакетов для отправки
            ...
            if len(back_node) < 2:
                ...
                for n in recs_target:
                    ...
                    info_target[n]['packets_id'].extend(list(msg_to_send))
    ...
    if local_need != set() and mode == 'CanRtr': # Есть пакеты, которых не хватает кластеру
        ...
        retrans_msg['data']['need_packets'] = list(local_need) # Записываю новый need_packets
...
def retranslation(msg, mode):
    ...
    if node_id in source_and_target:
        ...
        if mode == 'Fullset':
            ...
            if time.time() - msg_time < time_slot:
...
def Waiting(waiting_time, current_time):
    ...
    with buff_lock:
        ...
        process_request_packets(full_msg, 'CanRtr')
        ...
        try:
            ...
            if msg.get('type') == 'Packets':
                ...
                if msg['route'] != list():
                    ...
                    if 'data1' in msg:
                        # Получаем список соседей для ретрансляции (актуализированный)
                        Neibors = msg.get('data1') # Абоненты, которые учавствуют в передаче
                        pkts = list(msg.get('data')) # Список полученных пакетов
                        
                        msg_to_send = msg.copy()
                        
                        # Подгоняем структуру сообщения 
                        msg_to_send.pop('data') # (Отчищаем data_n)
                        msg_to_send.pop('data1') #

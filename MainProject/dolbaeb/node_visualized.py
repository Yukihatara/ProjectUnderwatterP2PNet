...
# Функция для отправки статуса визуализатору
def send_status_to_visualizer(node_id, packets, position, transmission=None):
    """Отправляет текущий статус узла визуализатору"""
    try:
        visualizer_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        status = {
            'node_id': node_id,
            'packets': list(packets.keys()) if isinstance(packets, dict) else list(packets),
            'position': position,
            'time': time.time()
        }
        if transmission:
            status['transmission'] = transmission
        visualizer_socket.sendto(json.dumps(status).encode(), ('127.0.0.1', 6000))
        visualizer_socket.close()
...
send_status_to_visualizer(node_id, packets, position)
...
def send_to(target_id, data, purpose_node, msg_type=None):
    ...
    if msg_type == 'PACKETS':
        ...
        # Отправляем статус о передаче визуализатору
        transmission = {
            'from': node_id,
            'to': target_id,
            'packets': list(data.keys()) if isinstance(data, dict) else data,
            'type': msg_type
        }
        send_status_to_visualizer(node_id, packets, position, transmission)
    ...
    while True:
        try:
            ...
            if msg.get('type') == 'PACKETS':
                ...
                # Отправляем обновленный статус визуализатору
                send_status_to_visualizer(node_id, packets, position)
            ...
            if msg.get('type') == 'BEACON':
                ...
                if sender not in network_status[node_id]['neibors']:
                    network_status[node_id]['neibors'][sender] = {}
                network_status[node_id]['neibors'][sender].update({'position': msg.get('position'), 'packets_id': msg.get('packets_id')})
...
def MainLoop():    
    ...
    while True:
        if warmup < 2:
            ...
            # print(*(f'  - {key}' for key in network_status[node_id]['neibors'].keys()))
    ...
    while True:
        # Отправляем статус визуализатору каждые 2 секунды
        send_status_to_visualizer(node_id, packets, position)
        ...
        time.sleep(2)  # Уменьшил до 2 секунд для более частого обновления
